...
/*   Updated: 2023/11/15 11:27:48 by hubrygo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_check_arounds(char **map, int y, int x)
{
	if (!y || !x || !map[y + 1] || !map[y][x + 1])
		return (1);
	if (isinset(map[y - 1][x - 1], "01SNEWP"))
		return (1);
	if (isinset(map[y - 1][x], "01SNEWP"))
		return (1);
	if (isinset(map[y - 1][x + 1], "01SNEWP"))
		return (1);
	if (isinset(map[y][x - 1], "01SNEWP"))
		return (1);
	if (isinset(map[y][x + 1], "01SNEWP"))
		return (1);
	if (isinset(map[y + 1][x - 1], "01SNEWP"))
